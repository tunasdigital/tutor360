...
export default function OffcanvasArea({
  ...
  // ✅ Evita mismatch SSR/Client: só renderiza o offcanvas depois do client montar
  ...
  // trava o scroll do body quando aberto (só no client)
  useEffect(() => {
    if (!mounted) return;
  ...
  }, [openOffCanvas, mounted]);

  // ✅ No SSR e antes do mount, não renderiza nada
  if (!mounted) return null;

  // ✅ Se está fechado, não renderiza nada (mantém HTML consistente)
  if (!openOffCanvas) return null;

  return (
    <div
      className={`tp-offcanvas-overlay ${openOffCanvas ? "opened" : ""}`}
      ...
      >
        ...
        <div className="tp-offcanvas-body">
          ...
          <p className="tp-offcanvas-text">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. A few quick
            links and contact details live here.
          </p>

          <nav className="tp-offcanvas-links">
            <Link href="/university-about" onClick={onHandleOffCanvas}>
              About
            </Link>
            <Link href="/course" onClick={onHandleOffCanvas}>
              Courses
            </Link>
            <Link href="/admissions" onClick={onHandleOffCanvas}>
              Admissions
            </Link>
            <Link href="/blog" onClick={onHandleOffCanvas}>
              Blog
            </Link>
            <Link href="/contact" onClick={onHandleOffCanvas}>
              Contact
            </Link>
          </nav>
